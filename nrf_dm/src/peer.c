...
static void print_result(struct dm_result *result)
{
	...
	if (result->ranging_mode == DM_RANGING_MODE_RTT) {
		...
		// // printk("{'addr': 'DD:F8:63:2F:91:E3', 'quality': 'ok', 'mcpd_high_precision': '0.55', 'mcpd_ifft': '0.56', 'mcpd_phase_slope': '0.55', 'mcpd_rssi_openspace': '0.55', 'mcpd_best': '0.56',}"):
		printk("\n\n");
		printk("{\"addr\": \"%s\", \"quality\": \"%s\", \"mcpd_high_precision\": \"%.2f\", \"mcpd_ifft\": \"%.2f\", \"mcpd_phase_slope\": \"%.2f\", \"mcpd_rssi_openspace\": \"%.2f\", \"mcpd_best\": \"%.2f\"}",
			addr,
			quality[result->quality],
			result->dist_estimates.mcpd.high_precision,
			result->dist_estimates.mcpd.ifft,
